...
export class WibbleError extends Error {
  constructor(public errors: common.Errors, public node: ast.PNode) {
    super(errors.inspect());
  }
}

function simplify(s: string, options: EngineOptions = {}): ast.PNode {
...
//   });
  ...
  it("new/on", () => {
    should.throws(() => simplify("new { 3 }"), (error: WibbleError) => {
  ...
  it("break", () => {
    simplify("repeat { break 10 }").source().should.eql("repeat (break 10)");
    should.throws(() => simplify("if true then { 10; break 11 }"), (error: WibbleError) => {
      error.node.source().should.eql("if true then { 10; break 11 } else ()");
      error.errors.inspect().should.match(/\[19:27\] 'break'/);
      return true;
    });
  });
